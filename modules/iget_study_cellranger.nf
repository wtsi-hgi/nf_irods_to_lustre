process 'iget_study_cellranger' {
    ...
    publishDir  "${params.outdir}/iget_study_cellranger/${clean_study_id}/${clean_run_id}/${clean_sample}",
      saveAs: {filename ->
  ...
  output:
    tuple val(study_id), val(sample), path("cr__*"), emit: study_sample_cellranger optional true
    tuple val(sample), env(FILT_DIR), env(FILT_BARCODES), env(FILT_H5), env(BAM_FILE), emit: cellranger_filtered_outputs optional true
    tuple val(sample), env(RAW_DIR), env(RAW_BARCODES), env(RAW_H5), env(BAM_FILE), emit: cellranger_raw_outputs optional true
    path("${sample}.metadata.tsv"), emit: cellranger_metadata_tsv optional true
    env(VERSION), emit: VERSION optional true
...
RESULTS_DIR=${params.outdir}/iget_study_cellranger/${clean_study_id}/${clean_run_id}/${clean_sample}
...
BAM_FILE=\$RESULTS_DIR/\$(find . | grep 'possorted.*bam.bam\$'  | cut -c 3-)
...
NAME=\$(find cr__\${VERSION}__\$cellranger_irods_name/ | grep summary.csv)
echo NAME of metadata is \$NAME
cat \$NAME | $workflow.projectDir/../bin/remove_comma_inside_quotes.sh  > metadata2.csv
paste -d, metadata1.csv metadata2.csv | tr ',' '\t' > ${sample}.metadata.tsv
cp ${sample}.metadata.tsv cr__\${VERSION}__\$cellranger_irods_name
rm metadata1.csv 
rm metadata2.csv 
} || { echo "Error parsing files"; exit 1; }
echo script done

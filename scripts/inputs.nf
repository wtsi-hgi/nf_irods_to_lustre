params {
    ...
    // Should add a run ID
    // We will perhaps additional input mode of retrieving samples from LIMS
    // SELECT * FROM mlwarehouse.study WHERE name LIKE '%Cardinal%'; 
    // The issue with this querry is that not all samples has a cardinal in the name. For example this one - SELECT * FROM mlwarehouse.study WHERE id_study_tmp LIKE '%6562%', which has been processed as part of the Cherry ELGH.

    // SELECT s.name, f.*
    // FROM mlwarehouse.iseq_flowcell f
    // JOIN mlwarehouse.study s ON f.id_study_tmp = s.id_study_tmp
    // WHERE s.id_study_tmp LIKE '%6463%'
    ...
    study_id_mode {
        ...
        input_studies = ['6634'] //       list of study_ids to pull, can be more than one (seperated by commas).
        input_runs = ['44622']
    }
    ...
    csv_samples_id_mode {
        ...
        input_samples_csv = "${launchDir}/input.tsv"
    ...
    google_spreadsheet_mode {
        ...
        input_sheet_name = "41510_Card_Val" // name of sheet if the spreadsheet has multiple sheets. Set to 'sheet1' for default.
